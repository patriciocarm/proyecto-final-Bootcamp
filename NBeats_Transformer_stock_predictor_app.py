# -*- coding: utf-8 -*-
"""
Created on Fri Jun 13 08:35:17 2025
...
class NBEATSBlock(nn.Module):
    def __init__(self, input_size, backcast_size, forecast_size):
        super().__init__()
        self.fc1 = nn.Linear(input_size, 64)
        self.fc2 = nn.Linear(64, 64)
        self.fc3 = nn.Linear(64, backcast_size + forecast_size)
...
class NBEATSModel(nn.Module):
    def __init__(self, input_size, backcast_size, forecast_size, num_blocks=3):
        super().__init__()
        self.blocks = nn.ModuleList([
            NBEATSBlock(input_size, backcast_size, forecast_size)
...
def guardar_modelo(modelo, scaler, ticker, modelo_nombre):
    os.makedirs("modelos_guardados", exist_ok=True)
    nombre_archivo = f"modelos_guardados/{ticker}_{modelo_nombre}.pt"
    torch.save({
        'model_state_dict': modelo.state_dict(),
        'scaler': scaler,
    }, nombre_archivo)
    st.info(f"Modelo guardado en {nombre_archivo}")

def cargar_modelo(ticker, modelo_nombre, modelo_clase, **kwargs):
    nombre_archivo = f"modelos_guardados/{ticker}_{modelo_nombre}.pt"
    if os.path.exists(nombre_archivo):
        checkpoint = torch.load(nombre_archivo, map_location=DEVICE)
        modelo = modelo_clase(**kwargs).to(DEVICE)
        modelo.load_state_dict(checkpoint['model_state_dict'])
        scaler = checkpoint['scaler']
        st.info(f"Modelo cargado desde {nombre_archivo}")
...
def ejecutar_pipeline(ticker: str, inicio: str, fin: str, dias_pred: int,
    ...
    if modelo == 'arima':
        ...
        model, scaler_cargado = cargar_modelo(
            ticker, 'transformer', TransformerModel,
            input_size=1, d_model=16, nhead=2, num_layers=1
        )
        if model is not None:
            st.success("Modelo Transformer cargado, saltando entrenamiento.")
            ultimos_datos_sequence = list(serie_escalada[-look_back:])
            pred_scaled = []
            model.eval()
            with torch.no_grad():
        ...
        try:
            X_train, Y_train = create_sequences(train_scaled, look_back)
            X_train = X_train.squeeze(-1)  # [batch, look_back]
            forecast_size = 1
            model = NBEATSModel(
                input_size=look_back,
                backcast_size=look_back,
                forecast_size=forecast_size,
                num_blocks=3
            ).to(DEVICE)
            criterion = nn.MSELoss()
            optimizer = optim.Adam(model.parameters(), lr=RNN_LEARNING_RATE)
            st.write(f"Entrenando modelo N-BEATS en {DEVICE}...")
            progress_bar = st.progress(0)
            model.train()
            for epoch in range(RNN_EPOCHS):
                optimizer.zero_grad()
            ...
            with torch.no_grad():
                for i in range(dias_pred):
                    x_input = torch.FloatTensor(ultimos_datos_sequence[-look_back:]).view(1, look_back).to(DEVICE)
                    yhat = model(x_input).item()
                    if i == 0:
                        first_predicted_scaled_debug = yhat
                        first_predicted_unscaled_debug = scaler.inverse_transform(np.array(first_predicted_scaled_debug).reshape(-1, 1)).flatten()[0]
            ...
            if len(test_scaled) > look_back:
                ...
                st.warning("Pocos datos en el conjunto de prueba para calcular MSE para N-BEATS.")
                mse_test = np.nan
        except Exception as e:
            st.error(f"Error al entrenar o predecir con N-BEATS: {e}")
            return None, None, None, None, None

    else:
        st.error("Modelo no soportado. Por favor, elige 'arima', 'rnn', 'lstm', 'transformer' o 'nbeats'.")
        return None, None, None, None, None
...
modelo = st.sidebar.selectbox(
    "Modelo a utilizar",
    ('arima', 'rnn', 'lstm', 'transformer', 'nbeats')
)
